...
输入拼音或英文字母时，小狼毫会先生成 Rime 原有候选，再异步向 Jev 发送最近已经上屏的文字、当前输入码和当前页前 10 个候选。Jev 根据前文语境返回每个安全选项的概率；高概率候选会稳定排到前面，概率相同时保持 Rime 原顺序。按空格时，第 1 位候选会映射回 Rime 的原位置后提交；Jev 也可以在安全校验通过时选择当前原始字母。模型不能提交选择集合以外的自由文本。
